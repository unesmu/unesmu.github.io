...
Further look into the time distribution of the quotations higlight how some periods are more favourable to negative or positive declarations on our topic. 
...
Separating the quotes into three categories, positive negative and neutral using thresholds on sentiment scores reveals
that around the end of 2017 and early 2018 there is an increase in the amount of negative quotes. Meanwhile on the rest of the time period, positive sentiments are occuring more frequently, reasssuring thoughts with respect to the previous analysis. 
Neutral quotes are also present but generally less than each of the other categories, as we have seen earlier. Since the threshold
used for splitting could affect the number of quotes in a given category we suggest here a visualization of all sentiment scores over time.
